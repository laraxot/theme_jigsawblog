@extends('pub_theme::layouts.app')

@php
//$page->type = 'article';
//dddx($row->categories);
@endphp

@section('content')
    @if ($page->cover_image)
        <img src="{{ $page->cover_image }}" alt="{{ $row->title }} cover image" class="mb-2">
    @endif

    <h1 class="leading-none mb-2">{{ $row->title }}</h1>
    <h2 class="text-gray-800 md:mt-0">{{ $row->subtitle }}</h2>

    <p class="text-gray-700 text-xl md:mt-0">
        {{ $row->author }} • {{ $row->updated_at->format('d/m/Y') }}
    </p>

    @if ($row->categories)
        @foreach ($row->categories as $i => $category)
            <a href="{{ '/blog/categories/' . $category }}" title="View posts in {{ $category }}"
                class="inline-block bg-gray-300 hover:bg-blue-200 leading-loose tracking-wide text-gray-800 uppercase text-xs font-semibold rounded mr-4 px-3 pt-px">{{ $category }}</a>
        @endforeach
    @endif


    <div class="border-b border-blue-200 mb-10 pb-4" v-pre>
        {{--
        @yield('content')

        --}}
        {!! $row->txt !!}
    </div>

    <nav class="flex justify-between text-sm md:text-base">
        <div>
            {{--
            @if ($next = $page->getNext())
                <a href="{{ $next->getUrl() }}" title="Older Post: {{ $next->title }}">
                    &LeftArrow; {{ $next->title }}
                </a>
            @endif
            --}}
        </div>

        <div>
            {{--
            @if ($previous = $page->getPrevious())
                <a href="{{ $previous->getUrl() }}" title="Newer Post: {{ $previous->title }}">
                    {{ $previous->title }} &RightArrow;
                </a>
            @endif
            --}}
        </div>
    </nav>
@endsection
